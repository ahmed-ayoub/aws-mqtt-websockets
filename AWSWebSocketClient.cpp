...
AWSWebSocketClient:
    ...
    awsToken = NULL;
    path = NULL;
    awsCustomAuthName = NULL;
    awsCustomHeaderName = NULL;
    awsCustomToken = NULL;
    awsCustomSignature = NULL;
    _connected = false;
...
AWSWebSocketClient::~AWSWebSocketClient(void)
{
    ...
    if (awsToken != NULL)
        delete[] awsToken;
    if (awsCustomAuthName != NULL)
        delete[] awsCustomAuthName;
    if (awsCustomHeaderName != NULL)
        delete[] awsCustomHeaderName;
    if (awsCustomToken != NULL)
        delete[] awsCustomToken;
    if (awsCustomToken != NULL)
        delete[] awsCustomSignature;
}
...
//generate AWS url path and WS headers, using custom authorizer
char *AWSWebSocketClient::generateAWSCustomHeaders()
{
    String canonicalUri = F("/mqtt");

    String custAuthName = awsCustomAuthName;
    String custAuthSign = awsCustomSignature;
    String custAuthToken = awsCustomToken;
    String custAuthHeader = awsCustomHeaderName;

    char extraHeaders[strlen(custAuthName.c_str()) + strlen(custAuthHeader.c_str()) + strlen(custAuthSign.c_str()) + strlen(custAuthToken.c_str()) + 256];
    sprintf(extraHeaders, "X-Amz-CustomAuthorizer-Name: %s\nX-Amz-CustomAuthorizer-Signature: %s\n%s: %s", custAuthName.c_str(), custAuthSign.c_str(), custAuthHeader.c_str(), custAuthToken.c_str());
    setExtraHeaders(extraHeaders);

    char *requestUri = new char[strlen(canonicalUri.c_str()) + 2]();
    sprintf(requestUri, "%s", canonicalUri.c_str());
    return (requestUri);
}

////// Custom Auth Setters

AWSWebSocketClient &AWSWebSocketClient::setAWSCustomAuthName(const char *awsCustomAuthName)
{
    if (this->awsCustomAuthName != NULL)
        delete[] this->awsCustomAuthName;
    int len = strlen(awsCustomAuthName) + 1;
    this->awsCustomAuthName = new char[len]();
    strcpy(this->awsCustomAuthName, awsCustomAuthName);
    return *this;
}

AWSWebSocketClient &AWSWebSocketClient::setAWSCustomHeaderName(const char *awsCustomHeaderName)
{
    if (this->awsCustomHeaderName != NULL)
        delete[] this->awsCustomHeaderName;
    int len = strlen(awsCustomHeaderName) + 1;
    this->awsCustomHeaderName = new char[len]();
    strcpy(this->awsCustomHeaderName, awsCustomHeaderName);
    return *this;
}

AWSWebSocketClient &AWSWebSocketClient::setAWSCustomToken(const char *awsCustomToken)
{
    if (this->awsCustomToken != NULL)
        delete[] this->awsCustomToken;
    int len = strlen(awsCustomToken) + 1;
    this->awsCustomToken = new char[len]();
    strcpy(this->awsCustomToken, awsCustomToken);
    return *this;
}

AWSWebSocketClient &AWSWebSocketClient::setAWSCustomSignature(const char *awsCustomSignature)
{
    if (this->awsCustomSignature != NULL)
        delete[] this->awsCustomSignature;
    int len = strlen(awsCustomSignature) + 1;
    this->awsCustomSignature = new char[len]();
    strcpy(this->awsCustomSignature, awsCustomSignature);
    return *this;
}
...
bool AWSWebSocketClient::getUseCustomAuth()
{
...
AWSWebSocketClient &AWSWebSocketClient::setUseCustomAuth(bool value)
{
    useCustomAuth = value;
    return *this;
}
